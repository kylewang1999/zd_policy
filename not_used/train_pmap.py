...
def main(cfg):
    ...
    if "warmstart" in cfg and cfg.warmstart is not None:
        ...
        vpi_params["psi_policy"] = ws_state.params["psi_policy"]
    ...
    def train_step(state, batch):
        ...
        aux = jax.tree_map(lambda x_: jnp.vstack(x), aux)
        loss = jnp.mean(loss)
    ...
    wandb.init(project="zero_dynamics_policies", name=f"{cfg.experiment_name}",
               config=cfg_dict)
...
if __name__ == "__main__":
    ...
    os.environ["XLA_FLAGS"] = f"--xla_force_host_platform_device_count={max(os.cpu_count(), 128)}"
